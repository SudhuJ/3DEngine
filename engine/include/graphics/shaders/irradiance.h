...
namespace flow {
    struct IrradianceShader : public Shader {
        FLOW_INLINE IrradianceShader(const std::string& vertFile, const std::string& fragFile)
        ...
        FLOW_INLINE uint32_t Generate(Texture& texture, skyboxMesh& mesh, int32_t size) {
            ...
            glCreateTextures(GL_TEXTURE_CUBE_MAP, 1, &irradMap);
            glTextureStorage2D(irradMap, 1, GL_RGB16F, size, size);

            glTextureParameteri(irradMap, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
            glTextureParameteri(irradMap, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
            glTextureParameteri(irradMap, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
            glTextureParameteri(irradMap, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
            glTextureParameteri(irradMap, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
            ...
            for (uint32_t i = 0; i < 6; i++) {
                ...
                glNamedFramebufferTextureLayer(FBO, GL_COLOR_ATTACHMENT0, irradMap, 0, i);
                glBindFramebuffer(GL_FRAMEBUFFER, FBO);
                glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
                renderSkyboxMesh(mesh);
            }

            glGenerateTextureMipmap(irradMap);
